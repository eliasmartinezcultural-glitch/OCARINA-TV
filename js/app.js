...
async function renderEpisode(){
 ...
 shell(e.titulo,'<main class="detail"><div class="kicker">'+esc(p?.nombre||'OCARINA TV')+' · '+esc(e.categoria)+'</div><h1>'+esc(e.titulo)+'</h1>'+buildPlayer(e,'episode')+'<p>'+esc(e.descripcion)+'</p><div class="meta"><span>Territorio: '+esc(e.territorio||'No especificado')+'</span><span>Fecha: '+esc(e.fecha||'No cargada')+'</span><span>Duración: '+esc(e.duracion||'No cargada')+'</span></div><div class="chips">'+(e.tags||[]).map(t=>'<span>'+esc(t)+'</span>').join('')+'</div><p class="muted">Programa: <a href="programas.html?id='+encodeURIComponent(e.programaId)+'">'+esc(p?.nombre||'Archivo Ocarina')+'</a> · Estado: '+esc(e.estado)+'</p><div class="episode-actions"><button id="episodeBackBtn" class="btn" type="button">← Volver a videos</button><button id="episodeRandomBtn" class="btn" type="button">▶ Ver otro video</button></div><section class="section"><div class="kicker">CONTENIDO RELACIONADO</div><h2>También puede interesarte</h2><div class="grid">'+related.map(card).join('')+'</div></section></main>');
 bindPlayerControls();mediaMetadata(e);
 document.getElementById('episodeBackBtn')?.addEventListener('click',()=>{
   const sameSite=document.referrer&&new URL(document.referrer,location.href).origin===location.origin;
   if(sameSite&&history.length>1){history.back();return}
